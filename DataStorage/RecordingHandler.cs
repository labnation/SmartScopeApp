...
namespace ESuite.DataStorage
{
    ...
    internal static class RecordingHandler
    {
        ...
        {
            string filename = C.Utils.GetTempFileName(".mat");
                            ...
                            if (acqData != null)
                            {
                                foreach (DecoderOutput decOut in acqData)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        values.Add((decOut as DecoderOutputValueNumeric).Value);
                                    }
                                }
                            }
        ...
        {
            string filename = C.Utils.GetTempFileName(".csv");
                ...
                {                    
                    if (pair.Value.BytesStored() > 0)
                    {
                        ...
                        for (int a = 0; a < recording.AcquisitionsRecorded; a++)
                        {
                            ...
                            if (acqData != null) //as this happened on crashreport 2017-09-16 11:11:57
                            {
                    ...
                    foreach (var kvp in buffers)
                    {
                        ...
                        {//DecoderOutput
                            if (acqData.Length >= i + 1)
                            {
                                ...
                                if (decOut is DecoderOutputValueNumeric)
                                    streamWriter.Write((decOut as DecoderOutputValueNumeric).Value.ToString());
                                streamWriter.Write(delimiter);
                            }
                            else
                            {
                    ...
                    { // for simple datatypes                        
                        for (int i = 0; i < recording.acqInfo.Count; i++)
                        {
                            ...
                            {// in case of decoder output
                                ...
                                var unsupportedOutputs = decOut.Where(x => !(x is DecoderOutputEvent || x is DecoderOutputValueNumeric)).ToList();
                                ...
                                { //only DecoderOutputEvent or DecoderOutputValue<byte>
                                    ...
                                    string values = String.Join(delimiter, (decOut).Select(x => (x is DecoderOutputEvent) ? "" : (x as DecoderOutputValueNumeric).Value.ToString()).ToArray());
                                    ...
                                    for (int x = 0; x < decOut.Length; x++)
                                    {
                                        ...
                                        if (decOut[x] is DecoderOutputValueNumeric)
                                            streamWriter.Write(": " + (decOut[x] as DecoderOutputValueNumeric).Value.ToString());
                                        streamWriter.Write(delimiter);
